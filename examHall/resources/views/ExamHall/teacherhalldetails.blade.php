...
<table>
    ...
    <form method="get" action="{{Route('viewte-halldetails')}}">
        @csrf
        <input type="hidden" value="{{$item->id}}" name="teacher-id"/>
    <tr>
      <td>{{ \App\User::where(['id'=>$item->user_id])->first()->name }}</td>
      <td> @if (\App\User::where(['id'=>$item->user_id])->first()->department==1)
        {{ 'CSE' }}
         
        @elseif(\App\User::where(['id'=>$item->user_id])->first()->department==2)
        {{ 'BBA' }}
  
        @elseif(\App\User::where(['id'=>$item->user_id])->first()->department==3)
        {{ 'LAW' }}
      ...
      <td>
        <a href="{{route('teacherhall-delete',['id'=>$item->id])}}" class="btn btn-danger btn-sm">Delete</a>
