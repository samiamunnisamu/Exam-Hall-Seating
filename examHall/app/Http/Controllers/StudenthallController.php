...
class StudenthallController extends Controller
{
    public function index(){
        ...
        $firstStudent = Crform::with(['users', 'course'])->where(['user_id' => Auth::id()])->get();
       //dd($firstStudent);
        return view('ExamHall.studenthall')->with([
    ...
    public function studentHall(Request $request){
        $halls = Hall::get();
        $exam = Exam::where(['course_id'=>$request['exam-id']])->orderBy('id', 'DESC')->first();
//dd($exam,$request['exam-id']);
        return view('ExamHall.studenthall2')->with([
            'halls' => $halls,
            'exam' => $exam
        ]);
    ...
    public function studentHall2(Request $request){
        $departmentArray = [];
...
//            dd($firstStudent);
        ...
        return view('ExamHall.studenthall3')->with([
            'sit_row'=>$sitrow,
